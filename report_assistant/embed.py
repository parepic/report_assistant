...
from report_assistant.utils.load_utils import get_index_path, load_chunks, load_document_entry, load_global_config
from report_assistant.utils.qdrant_utils import (
    get_embedding_dimension,
    get_qdrant_client,
    create_collection_if_missing,
    check_and_handle_existing_points,
    embed_chunks,
    create_payload_indexes_if_missing,
    slugify_name,
)
...
def main(config: GlobalConfig, mode="chatbot") -> None:
    ...
    proceed = check_and_handle_existing_points(client, collection_name, chunks_file.strategy_hash, entry.doc_id)
    if proceed:
        ...
        upsert_to_company_collection(client, collection_name, chunks, vectors, chunks_file, entry)
